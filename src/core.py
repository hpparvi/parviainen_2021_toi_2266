...
def read_tess(dfiles: List[Path], zero_epoch: float, period: float, use_pdc: bool = False,
...
def read_m2(files: list, downsample=None, passbands=('g', 'r', 'i', 'z_s')):
...
# HiPERCAM routines
# -----------------
def read_hipercam(files: list, downsample=None, passbands=('g', 'r', 'i', 'z')):
    times, fluxes, pbs, wns, covs = [], [], [], [], []
    for inight, f in enumerate(files):
        with pf.open(f) as hdul:
            npb = (len(hdul)-1)//2
